    ...
    const fetchDish = async () => {
      try {
        ...
        if (found) {
          ...
          const completedOrdersWithDish = orders.filter(
            (o) =>
              o.status === "Completed" &&
              o.items &&
              o.items.some((i) => i.id === found.id),
          );
          ...
          // Tính toán rating
          const ratedOrders = completedOrdersWithDish.filter(
            (o) => o.rating > 0,
          );
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="sticky top-0 z-30 lg:hidden bg-white/80 backdrop-blur-md border-b border-gray-100 px-4 py-3 flex items-center justify-between">
        ...
        <span className="font-bold text-textMain truncate max-w-[200px]">
          {dish.name}
        </span>
        <button
          ...
          <Heart
            size={24}
            fill={wishlist.includes(dish.id) ? "currentColor" : "none"}
          />
        </button>
      </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-6">
            <div className="relative aspect-[4/3] rounded-[2rem] overflow-hidden shadow-2xl group">
              ...
              <button
                onClick={() => toggleWishlist?.(dish.id)}
                className={`absolute top-6 right-6 w-12 h-12 rounded-2xl flex items-center justify-center shadow-xl transition-all hidden lg:flex ${
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-2xl border border-gray-100 flex flex-col items-center gap-1 shadow-sm">
                ...
                <span className="text-xs text-textSec font-medium">
                  Lượt bán
                </span>
                <span className="text-sm font-bold">
                  {salesCount > 0 ? `${salesCount}+` : "Mới"}
                </span>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-gray-100 flex flex-col items-center gap-1 shadow-sm">
              ...
              <div className="bg-white p-4 rounded-2xl border border-gray-100 flex flex-col items-center gap-1 shadow-sm">
                <Star
                  className={
                    ratingData.count > 0 ? "text-yellow-500" : "text-gray-300"
                  }
                  size={20}
                  fill={ratingData.count > 0 ? "#f59e0b" : "currentColor"}
                />
                <span className="text-xs text-textSec font-medium">
                  Đánh giá
                </span>
          ...
          <div className="flex flex-col h-full lg:pt-4">
            <div className="hidden lg:flex items-center gap-4 mb-6">
              ...
              <span className="text-sm font-bold text-primary truncate max-w-[250px]">
                {dish.name}
              </span>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 lg:p-8 border border-gray-100 shadow-sm mb-auto">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div>
                  <span className="text-textSec text-sm font-medium mb-1 block">
                    Giá hiện tại
                  </span>
                ...
                <div
                  className={`flex items-center gap-4 bg-background p-2 rounded-2xl border border-gray-100 ${dish.isAvailable === false ? "opacity-40 pointer-events-none" : ""}`}>
                  <button
                  ...
                  <span className="w-8 text-center font-bold text-lg">
                    {dish.isAvailable === false ? 0 : quantity}
                  </span>
                  <button
            ...
            <div className="mt-8 flex items-start gap-3 p-4 bg-orange-50/50 rounded-2xl border border-orange-100/50">
              ...
              <p className="text-xs text-textSec leading-normal">
                Món ăn này được chế biến từ nguyên liệu tươi sống trong ngày.
                Vui lòng ghi chú nếu bạn có bất kỳ dị ứng nào với thành phần của
                món ăn.
              </p>
            </div>
          </div>
