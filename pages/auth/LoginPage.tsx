    ...
    <div className="min-h-screen flex w-full">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gray-900">
        ...
        <div className="relative z-10 flex flex-col justify-end p-16 h-full text-white">
          <h1 className="text-5xl font-bold mb-4">Chào mừng quay trở lại!</h1>
          <p className="text-xl opacity-90 max-w-md">Khám phá hàng ngàn món ăn ngon và ưu đãi độc quyền.</p>
        </div>
      </div>
          ...
          <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div className="space-y-2">
                ...
                <input type="password" id="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full rounded-xl border-gray-200 bg-background h-12 px-4 focus:ring-primary focus:border-primary" required />
              </div>
              {error && <p className="text-sm text-red-500 font-semibold">{error}</p>}
              <div className="flex justify-end">
                <button type="button" className="text-primary text-sm font-bold hover:underline" onClick={() => navigate('/forgot-password')}>
                  Quên mật khẩu?
                </button>
              </div>
              <button type="submit" className="w-full bg-primary text-white font-bold h-12 rounded-xl hover:bg-primaryDark transition-all">
                Đăng nhập
              </button>
            </form>
